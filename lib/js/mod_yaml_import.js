var mod_yaml_import = (function ($) {
    ...
    */
    ...
    populateJson = function() {
        $(SEL_FORM).find("[name=json]").val(viewModel.toJSON());
    }
    ...
    */
    ...
    initUI = function() {
        $(SEL_FORM).find("input[type=file]").bootstrapFileInput();
        $(SEL_FORM).find("input[type=file]").on("change", function() {
            clearJson();
            $(SEL_FORM).submit();
        });
        activateStep("upload");
    ...
    initSettingsUI = function() {
        $(SEL_BUTTON_SUBMIT).on("click", function(e) {
            e.preventDefault();
            e.stopPropagation();
            submitForm(e);
        });
